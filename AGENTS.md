...
*   **Fallback Logic**:
    *   **Search**: Attempts direct iTunes API first, then falls back to proxy.
    *   **RSS Fetch**: Uses a **Race Strategy** (`Promise.any`) to attempt Direct Fetch (fastest) and Proxy Fetch (most reliable) simultaneously. This eliminates waiting for timeouts when CORS blocks direct access.
